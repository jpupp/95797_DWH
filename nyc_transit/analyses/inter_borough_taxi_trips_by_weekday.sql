...
JOIN {{ ref('mart__dim_locations')}} AS pu_loc ON all_taxi_trips.PUlocationID = pu_loc.LocationID
JOIN {{ ref('mart__dim_locations')}} AS do_loc ON all_taxi_trips.DOlocationID = do_loc.LocationID
)
